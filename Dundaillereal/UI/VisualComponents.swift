...
final class GameCoverCard: PaperCard {
    init(_ definition: Definition, compact: Bool = false, action: (() -> Void)? = nil) {
        ...
        let artHeight = art.heightAnchor.constraint(equalToConstant: compact ? 145 : 228)
        ...
        if !compact {
            heading.addArrangedSubview(styledLabel(definition.template.subtitle, .subheadline))
        }
        heading.translatesAutoresizingMaskIntoConstraints = false
        art.addSubview(heading)
        let shade = UIView()
        shade.backgroundColor = Theme.paper.withAlphaComponent(0.9)
        ...
        footer.layoutMargins = UIEdgeInsets(top: 12, left: 14, bottom: 14, right: 14)
...
extension Page {
    ...
    func playerBadges(_ session: Session) {
        ...
        for (i, p) in session.players.enumerated() {
            let card = PaperCard(inset: 12)
            let active = i == session.current
            card.backgroundColor = active ? Theme.coral.withAlphaComponent(0.13) : Theme.paper
            card.layer.borderColor =
                (active ? Theme.coral : Theme.sage.withAlphaComponent(0.25)).cgColor
            card.content.addArrangedSubview(
                styledLabel("\(active ? "▶" : "○") \(p.name)   \(p.score) 分", .subheadline))
